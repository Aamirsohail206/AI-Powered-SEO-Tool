...
def find_compitators(keyword, country="us", num_results=10):
    ...
    last_error = None
    attempts = [
        {"region": country_code, "num_results": max_results},
        {"country": country_code, "num_results": max_results},
        {"num_results": max_results},
        {"tld": "com", "lang": "en", "num": max_results, "stop": max_results, "pause": 2},
    ]

    for kwargs in attempts:
        try:
            results = list(search(keyword, **kwargs))
            if results:
                break
        ...
        try:
            ...
            with DDGS() as ddgs:
                for r in ddgs.text(keyword, max_results=max_results):
                    href = (r.get("href") or r.get("url") or "") if isinstance(r, dict) else ""
                    if href:
    ...
    if not competitors:
        extra = ""
        if last_error:
            extra = f" Details: {str(last_error)}"
        return {
            "error": "No competitors found. Google often blocks automated scraping. "
                     "Try again later, reduce requests, or install 'duckduckgo_search' for fallback." + extra
        }

    # Store competitor domains in DB
    store_competitor_data(keyword, competitors, country)
