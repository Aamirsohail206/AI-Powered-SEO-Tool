...
def traffic_results():
    ...
    chart_data, data = get_traffic_history(website)

    if isinstance(data, dict) and data.get("error"):
        return render_template("traffic_results.html", error=data.get("error"), website=website, data={})

    session['traffic_data'] = compress_data(data)

    return render_template(
        "traffic_results.html",
        data=data,
        website=website,
        traffic_history=json.dumps(chart_data)
    )
...
def generate_blog():
    ...
    try:
        ...
        image_url,image_url1 = None, None
...
def compare_traffic():
    if request.method == "POST":
        ...
        error1 = data1.get("error") if isinstance(data1, dict) else None
        error2 = data2.get("error") if isinstance(data2, dict) else None

        insights = None
        if not error1 and not error2 and data1 and data2:
        ...
        return render_template("compare_traffic_result.html",
                               ...
                               history1=json.dumps(history1 or {}),
                               history2=json.dumps(history2 or {}),
                               error1=error1,
                               error2=error2,
                               insights=insights)
