...
def fetch_backlinks(input_web):
    ...
    parsed = urlparse(input_web if "://" in input_web else f"http://{input_web}")
    domain = (parsed.netloc or "").strip()
    if not domain:
        domain = (parsed.path.split("/")[0] if parsed.path else "").strip()
    domain = domain.split(":")[0].strip().lower()

    if not domain:
        return {"error": "Invalid website/domain."}

    url = "https://website-analyze-and-seo-audit-pro.p.rapidapi.com/backlinks.php"
    querystring = {"domain": domain}

    rapidapi_key = os.getenv('API_KEY2')
    if not rapidapi_key:
        return {"error": "RapidAPI key not configured. Set RAPIDAPI_KEY (or API_KEY) in your environment."}
    headers = {
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            top_10 = (raw.get("backlinks") or [])[:10]
            backlinks_data = {
                "url": input_web,
                "domain": domain,
                "counts": raw.get("counts"),
                "backlinksList": top_10
            }
